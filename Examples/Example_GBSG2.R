...
#predict event probability @ t= 10
...
# internal validation with 10-fold CV
...
#internal validation with 5-fold CV (and 3-fold for model tuning)
...
for (i in 1:5) rpart.plot(ens2a_cv$model_list[[i]]$treemodel)

for (i in 1:5) print (ens3_cv$model_list[[i]]$treemodel)
# => it always splits by pnodes at 3.5 or 4.5 threshold 
...
#Finally, we can check the trees in ensemble 2 to see if there are any other non-linearities being picked up
